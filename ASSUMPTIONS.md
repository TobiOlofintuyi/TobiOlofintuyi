...
[2026-07-02 14:55] | **fly.io deploy scaffolded with fail-closed basic auth** | Committed Dockerfile + fly.toml + nginx config where the container refuses to start without `BASIC_AUTH_USER`/`BASIC_AUTH_PASS` secrets. App name `clause-ink-preview`, region `sea` (Seattle — closest to Tobi). | Tobi confirmed fly.io + sprites.dev as his platforms; the private-only requirement stands, and fail-closed is the only auth posture that can't be forgotten. This runner has no fly auth, so execution stays on the Mac. | Tobi renames the app / changes region at `fly launch`; a Tailscale/Flycast-private setup would be even tighter if he wants no public hostname at all.

[2026-07-02 14:55] | **Official DROP page supersedes June news figures** | Hub now says "over 600" registered brokers (the state's own current claim), keeps ~545-at-launch and 300K+ filers as trajectory; walkthrough rewritten to the page's actual mechanics (Identity Gateway/Login.gov, name+DOB+ZIP minimum, MAID/CTV/VIN optional, DROP ID as the official receipt, five statuses, 90-day reporting, broker-list narrowing, first-party/exempt/public data exclusions). | Tobi pasted the live privacy.ca.gov/drop content — primary source beats the 581 figure from June news coverage. The DROP-ID-as-receipt detail strengthens the receipts thesis natively. | The next official page revision; re-check at each copy pass.
